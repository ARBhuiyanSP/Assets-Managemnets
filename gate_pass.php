                ...
                <div class="content">
					...
					<div class="row" id="div1">
						<div class="table-responsive">
							...
							<tbody>
								<tr>
									<td><select name="item_name[]" class="form-control name_list">
									...
									<td><select name="product_name[]" class="form-control name_list">
												<option>Select Item</option>
													<?php 
									...
									<td><input type="text" name="quantity[]" id="quantity" placeholder="Enter your Qty" class="form-control name_list" /></td>
									<td><input type="text" name="product_description[]" id="product_description" placeholder="Enter your Rate" class="form-control name_list" /></td>
...
$(document).ready(function(){
    $('#add').click(function(){
        i++;
        $('#dynamic_field').append('<tr id="row'+i+'"><td><select name="item_name[]" class="form-control name_list"><option>Select Item</option><?php $sql	= "select * from products ORDER BY product_code ASC";$result = mysqli_query($link, $sql);while($row=mysqli_fetch_array($result)){?><option value="<?php echo $row["product_code"] ?>"><?php echo $row["product_name"] ?></option><?php } ?></select></td><td><select name="product_name[]" class="form-control name_list"><option>Select Item</option><?php $sql	= "select * from ams_products ORDER BY id ASC";$result = mysqli_query($link, $sql);while($row=mysqli_fetch_array($result)){?><option value="<?php echo $row['id'] ?>"><?php echo $row['item_name'] ?></option><?php } ?></select> </td><td><input type="text" name="quantity[]" placeholder="Enter your Qty" class="form-control name_list" /></td><td><input type="text" name="product_description[]" placeholder="Enter product description" class="form-control name_list" /></td><td><button type="button" name="remove" id="'+i+'" class="btn btn_remove" style="background-color:#f26522;color:#ffffff;">X</button></td></tr>');
    });
