                    ...
                    <div class="row" id="div1" style="background-color:#2e3192;color:#ffffff;">
						<div class="col-xs-2">
							<div class="form-group">
								...
								<?php
							$sqldata	=	"SELECT * FROM `gate_pass` ORDER BY `id` DESC LIMIT 1;";
							$resultdata = mysqli_query($link, $sqldata);
							$rowdata=mysqli_fetch_array($resultdata);
							
							$d = $rowdata['challan_no'];
								for ($n=0; $n<1; $n++) { ?>
									<input name="challan_no" class="form-control" value="<?php echo ++$d . PHP_EOL; ?>" readonly />
								<?php } ?>
									...
									<td width="80%">
										<select name="product_name[]" class="form-control name_list select2">
											...
											while($row=mysqli_fetch_array($result))
												{
											?>
											<option value="<?php echo $row['id'] ?>"><?php echo $row['item_name'] ?> || <?php echo $row['manu_sl'] ?> || <?php echo $row['assets_description'] ?></option>
											<?php } ?>
										</select>
									</td>
									<input type="hidden" name="qty[]" id="qty0" placeholder="Enter your Qty" class="form-control name_list" value="1" />
									<input type="hidden" name="description[]" id="description0" placeholder="Description here" class="form-control name_list"  required />
...
$(document).ready(function(){
    $('#add').click(function(){
        i++;
        $('#dynamic_field').append('<tr id="row'+i+'"><td><select name="product_name[]" class="form-control name_list select2"><option>Select Item</option><?php $sql	= "select * from `ams_products` where `rent_status`!='1' ORDER BY id ASC";$result = mysqli_query($link, $sql);while($row=mysqli_fetch_array($result)){?><option value="<?php echo $row["id"] ?>"><?php echo $row["item_name"] ?> || <?php echo $row["manu_sl"] ?></option><?php } ?></select></td><input type="hidden" name="qty[]" id="qty'+i+'" placeholder="Enter your Qty" class="form-control name_list" /><input type="hidden" name="description[]" id="description'+i+'" placeholder="Description here" class="form-control name_list" /><td><button type="button" name="remove" id="'+i+'" class="btn btn_remove" style="background-color:#f26522;color:#ffffff;">X</button></td></tr>');
